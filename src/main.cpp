...
// I2S pin configuration - matches your wiring
#define I2S_BCLK_PIN    12  // GP19 --> BCLK
#define I2S_LRC_PIN     14  // GP21 --> LRC (Word Select)
#define I2S_DOUT_PIN    13  // GP18 --> DIN
...
#define TEST_FREQUENCY  440  // A4 note (440 Hz)
#define AMPLITUDE       30000 // Amplitude (adjust for volume)
...
void loop() {
    ...
    int16_t audio_buffer[BUFFER_SIZE * 2]; // Stereo buffer (left + right channels)
...
// Alternative test function - call this from loop() for different test patterns
void playTestPattern() {
    ...
    for (int cycles = 0; cycles < 100; cycles++) {
        ...
        static float phase = 0.0;
        float phase_increment = 2.0 * M_PI * 1000 / SAMPLE_RATE; // 1kHz tone
